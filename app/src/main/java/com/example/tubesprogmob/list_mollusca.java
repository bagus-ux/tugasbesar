...
public class list_mollusca extends AppCompatActivity {
    RecyclerView recyclerView;
    RecyclerView.Adapter recylclerViewAdapter;
    RecyclerView.LayoutManager recyleViewLayaoutManager;
    ArrayList<molluscamodel> models;
    listmolluscaadapter AdapterecyleView;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_list_mollusca);
        ...
        for (int i = 0; i < datamollusca.nama.length; i++) {
            models.add(
                    new molluscamodel(
                            datamollusca.nama[i],
                            datamollusca.Keterangan[i],
                            datamollusca.gambar[i]));
        }

        recylclerViewAdapter = new listmolluscaadapter(models, this);
