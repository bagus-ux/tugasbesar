...
public class list_reptile extends AppCompatActivity {
    RecyclerView recyclerView;
    RecyclerView.Adapter recylclerViewAdapter;
    RecyclerView.LayoutManager recyleViewLayaoutManager;
    ArrayList<reptilemodel> models;
    listreptileadapter AdapterecyleView;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_list_reptile);
        ...
        for (int i = 0; i < datareptile.nama.length; i++) {
            models.add(
                    new reptilemodel(
                            datareptile.nama[i],
                            datareptile.Keterangan[i],
                            datareptile.gambar[i]));
        }

        recylclerViewAdapter = new listreptileadapter(models, this);
